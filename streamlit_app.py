...
with tab_inter:
    ...
    for k, idx in enumerate(options):
        m = MEALS[idx]
        with cols[k]:
            title_line = f"{k+1}) {m['name']}"
            st.markdown(f"**{title_line}**")
            comps = env.reward_components_if(idx)
            st.caption(
                f"Q={qs[idx]:.2f} | R≈{comps['reward']:.2f} "
...
"
    ...
    if st.button("Confirm choice"):
        ...
        from meal_planner_rl import QLearningAgent  # for type clarity; already imported
